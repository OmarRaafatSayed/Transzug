    ...
    <section className="container mx-auto px-4 sm:px-8 py-12 sm:py-20 bg-[#0d1220]">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center mb-12 lg:mb-16">
        <div>
          ...
          <p className="text-base sm:text-xl text-gray-300 mb-5 leading-relaxed">{t('subtitle')}</p>
          <p className="text-sm sm:text-lg text-gray-300 mb-8 leading-relaxed">{t('description')}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link href={`/${locale}/leistungen/lkw-logistik`}
              className="border-2 border-gray-700 text-white px-6 py-3 rounded-xl hover:bg-gray-800 transition-all font-semibold text-center">
              {t('cta.secondary')}
            </Link>
          </div>
        </div>
        <div className="relative">
          ...
          <div className="grid grid-cols-3 gap-3">
            {specs.map((spec, index) => (
              <div key={index} className="bg-[#151b28] border-2 border-gray-800 rounded-xl p-3 sm:p-6 text-center">
                <div className="text-xl sm:text-3xl font-bold mb-1 text-white">{t(spec.labelKey)}</div>
                <div className="text-xs sm:text-sm text-gray-400">{t(spec.sublabelKey)}</div>
              </div>
            ))}
          </div>
        </div>
