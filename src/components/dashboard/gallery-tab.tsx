...
export function GalleryTab() {
  ...
  const [newImage, setNewImage] = useState({ src: '', alt: '', category: 'نقل خاص' });
  ...
  async function addImage() {
    ...
    // Reset
    setNewImage({ src: '', alt: '', category: 'نقل خاص' });
    setImageFile(null);
    setImagePreview('');
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-lg font-bold text-white">إدارة الغاليري</h2>
          <p className="text-gray-400 text-sm">{images.length} صورة إجمالاً</p>
        </div>
        <button
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-500">
          ...
          <p className="text-sm">لا توجد صور في هذه الفئة</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((img) => (
            <div
              key={img.id}
              className="group relative bg-[#151b28] rounded-xl overflow-hidden border border-gray-800 hover:border-orange-600 transition-all"
            >
              {/* Image */}
              <div
              ...
              <div className="p-3 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-orange-500 text-[10px]">{img.category}</span>
                </div>
                <button
      ...
      {lightboxImg && (
        ...
        >
          ...
          <div className="absolute bottom-6 text-center">
            ...
            <p className="text-orange-500 text-sm">{lightboxImg.category}</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-[#151b28] border border-gray-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-bold text-lg">إضافة صورة جديدة</h3>
              <button
                onClick={resetModal}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="مثال: نقل أثاث منزلي"
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1.5">الفئة</label>
                <select
